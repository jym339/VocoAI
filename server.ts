...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    const systemInstruction = `You are "Kore", the VocoAI Backup Dispatcher, an elite automated voice answering assistant for "Apex Comfort HVAC", a premium commercial industrial heating, ventilation, and air conditioning contractor.
Your objective is to answer professional inbound calls, collect essential triage details for emergency or routine repairs, and demonstrate VocoAI's incredible capabilities.
...
If they say something out-of-bounds or irrelevant, politely guide them back to diagnosing their HVAC system.`;
    ...
    // Map messages payload to Gemini dynamic contents list
...
// (We provide transient memory/local state save, plus save endpoint for validation)
const leadDb: any[] = [];
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`VocoAI Server running at http://localhost:${PORT} in ${process.env.NODE_ENV || "development"} mode.`);
  });
}
